...
from .chess_helpers import chess_lib_color_to_player_side, move_to_uci

if TYPE_CHECKING:
    from .types import FEN, Move, PlayerSide
...
def calculate_fen_before_move(
    *,
    ...
    piece = chess_board.piece_at(chess_move.to_square)
    ...
    if piece is None:
        ...
        raise ValueError(
            ...
            f"no piece at square '{chess.SQUARE_NAMES[chess_move.from_square]}'."
    ...
    if piece_player_side != moving_player_side:
        raise ValueError(
            f"Invalid bot first move '{move_uci}': "
            f"piece at square '{chess_move.from_square}' is not "
            f"from player side '{moving_player_side}'"
        )

    # Now let's go back in time and rewind that first move...
    chess_board.remove_piece_at(chess_move.to_square)
    chess_board.set_piece_at(chess_move.from_square, piece)
