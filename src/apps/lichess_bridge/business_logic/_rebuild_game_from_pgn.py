...
class RebuildGameFromPgnResult(NamedTuple):
...
def rebuild_game_from_pgn(
    *, pgn_game: "chess.pgn.Game", factions: "GameFactions"
) -> RebuildGameFromPgnResult:
    ...
    _logger.info(
        "`rebuild_game_from_pgn` took %d ms. for %d moves",
        (time.monotonic() - start_time) * 1000,
        len(uci_moves),
    )

    return RebuildGameFromPgnResult(
        chess_board=chess_board,
        teams=teams,
        piece_role_by_square=piece_role_by_square,
        moves=uci_moves,
