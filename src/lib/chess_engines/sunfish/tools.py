...
def search(searcher, pos, secs, history=()):
    ...
    start = time.time()
    for i, (depth, move, score) in enumerate(searcher.search(pos, history)):
        if time.time() - start > secs:
            break
    return i, move, score, depth
...
def parseSAN(pos, msan):
    ...
    for (i, j), _ in gen_legal_moves(pos):
        ...
        if pos.board[i] == p and re.match(dst, cdst) and re.match(src, csrc):
            return (i, j)
    assert False, "Couldn't find legal move matching {}. Had {}".format(
        msan, {"p": p, "src": src, "dst": dst, "mvs": list(gen_legal_moves(pos))}
    )
