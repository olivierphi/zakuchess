...
import type {PieceRole, Square, Faction, PieceName, BoardOrientation} from "./types";
import {
...
import { useChessArenaStore } from "./ChessArenaProvider";
import { CHESS_PIECE_Z_INDEXES } from "./consts.ts";
...
const ChessCharacterDisplay: React.FC<ChessCharacterDisplayProps> = ({
  ...
  const { selectedSquare, availableMoves, currentPlayer } = useChessArenaStore((state) => ({
  ...
  const isPotentialCapture = Boolean(selectedSquare && square && availableMoves.includes(square));
