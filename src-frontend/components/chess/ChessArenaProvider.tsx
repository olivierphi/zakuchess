...
import type { Square, PieceRole, Faction } from './types';
...
function chessBoardToPieceRoleBySquare(chess: Chess): Partial<Record<Square, PieceRole>> {
  ...
  return create<ChessGameStore>((set, get) => {
    ...
    return {
      ...
      setFen: (fen: string) => {
        ...
        set({ fen, chess, pieceRoleBySquare, currentPlayer, selectedSquare: null, availableMoves: [] });
      },
...
export function useChessArenaStore<T>(selector: (state: ChessGameStore) => T): T {
  const store = useContext(ChessArenaContext);
  if (!store) {
    throw new Error('useChessArenaStore must be used within ChessArenaProvider');
  }
...
export function useChessArenaActions() {
  const store = useContext(ChessArenaContext);
  if (!store) {
    throw new Error('useChessArenaActions must be used within ChessArenaProvider');
  }
