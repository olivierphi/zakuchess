    ...
    theme: {
        ...
        extend: {
            ...
            dropShadow: {
                ...
                "potential-capture": borderFromDropShadow(
                    PIECES_DROP_SHADOW_OFFSET,
                    POTENTIAL_CAPTURE_COLOR,
                ),
...
function borderFromDropShadow(offset: number, color: string, unit: string = "px") {
    const dropShadow = []
    for (const [x, y] of [
        [offset, offset],
        [-offset, offset],
    ...
    return Object.fromEntries(
        FACTIONS.map((faction) =>
            PIECE_NAMES.map((pieceName) => [
                `${faction}-${pieceName}`,
                `url('/img/chess/units/${faction}/${pieceName}.png')`,
            ]),
        ).flat(),
    )
}
    ...
    return Object.fromEntries(
        PLAYER_SIDES.map((playerSide) =>
            PIECE_NAMES.map((pieceName) => [
                ...
                `url('/img/chess/symbols/${playerSide}-${pieceName}.svg')`,
            ]),
        ).flat(),
    )
}
...
function chessRelatedClassesSafeList() {
    ...
    const BODY_CLASS = "bg-body-background"

    return [BODY_CLASS, ...COLUMNS_CLASSES, ...RANK_CLASSES, ...SQUARE_COLORS]
}
